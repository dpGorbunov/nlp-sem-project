...
\begin{itemize}
    ...
    \item Multiclass Head: GPT-4, Llama, Gemma, unknown (detected via OOD methods)
\end{itemize}
...
\begin{enumerate}
    ...
    \item Shared Layer: Linear layer with tanh activation and dropout
...
\begin{itemize}
    \item Binary Head: human (0) vs AI (1)
    \item Multiclass Head: GPT-4, Llama, Gemma, unknown (detected via Mahalanobis)
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Practical deployment}: ruBERT-tiny2 (118MB, 29M params) with Mahalanobis achieves 85.25\% accuracy — only 4.7\% below Qwen, but with \textbf{20x faster inference} ($\sim$15ms CPU vs $\sim$300ms GPU) and \textbf{127x smaller} model size. This enables real-time CPU deployment without GPU.
\end{enumerate}
